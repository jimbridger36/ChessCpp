...
struct Move{
    ...
    Move(int _piece, int _start, int _end){
        piece = _piece;
        start = _start;
        end = _end;
    }
...
struct individualEntry {
    ...
    bool inline isNull(char type){
        ...
        if (type == 2){
            return val and eval == 0;
        }
        else if(type == 0){
            return val and eval == SCHAR_MIN;
        }
        else if(type == 1) {
            return val and eval == SCHAR_MAX;
        }
        else{
            return false;
        }
...
struct Entry{
    ...
    void setEval(int depth, int eval, int boundFlag){
        ...
        entries[boundFlag].depth = (u_char)depth;
    }
...
typedef std::unordered_map<u_long, Entry> mapType;
...
bool inline isNull(Entry& entry){
    return entry.entries[0].isNull(0) and entry.entries[1].isNull(1) and entry.entries[2].isNull(2) and isNull(entry.bestMove);
}
